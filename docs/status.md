...
where \(r_t(\theta)\) is the probability ratio and \(A_t\) is the advantage estimate computed using Generalized Advantage Estimation (GAE). The total loss combines policy loss, value function loss, and entropy regularization to balance performance and exploration.
